...
function initDatabase() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS routers (
      ...
      user_profile TEXT DEFAULT 'inforcenter-default',
  ...
  try { db.exec('ALTER TABLE leads ADD COLUMN ixc_cliente_id TEXT'); } catch (_) {}
  try { db.exec('ALTER TABLE leads ADD COLUMN is_client INTEGER DEFAULT 0'); } catch (_) {}
  ...
  `);
  insertSetting.run('hotspot_title', 'Wi-Fi Inforcenter');
  ...
  insertSetting.run('client_free_time', '480');
  ...
  insertSetting.run('redirect_url', 'https://inforcenterfibra.com.br');
  insertSetting.run('rate_limit_down', '10');
  insertSetting.run('rate_limit_up', '10');
  ...
  if (!adminExists) {
    const hash = bcrypt.hashSync(process.env.ADMIN_PASSWORD || 'inforcenter@2026', 10);
