...
router.get('/dashboard', (req, res) => {
  ...
  const totalClients = db.prepare('SELECT COUNT(*) as count FROM leads WHERE is_validated = 1 AND is_client = 1').get();
...
router.get('/leads', (req, res) => {
  ...
  if (search) {
    where += ' AND (name LIKE ? OR phone LIKE ? OR email LIKE ? OR cpf LIKE ?)';
  ...
  const leads = db.prepare(`
    SELECT id, name, phone, email, cpf, is_client, mac_address, ip_address, created_at,
...
router.get('/leads/export', (req, res) => {
  const leads = db.prepare(`
    SELECT name, phone, email, cpf, is_client, mac_address, ip_address, created_at
    FROM leads WHERE is_validated = 1
    ORDER BY created_at DESC
  `).all();

  const headers = 'Nome,Telefone,Email,CPF,Cliente Inforcenter,MAC,IP,Data Cadastro\n';
  const rows = leads.map(l =>
    `"${l.name || ''}","${l.phone || ''}","${l.email || ''}","${l.cpf || ''}","${l.is_client ? 'Sim' : 'Não'}","${l.mac_address || ''}","${l.ip_address || ''}","${l.created_at}"`
  ).join('\n');

  res.setHeader('Content-Type', 'text/csv; charset=utf-8');
  res.setHeader('Content-Disposition', 'attachment; filename="leads_inforcenter.csv"');
...
router.put('/settings', async (req, res) => {
  const allowed = [
    'hotspot_title', 'hotspot_subtitle', 'hotspot_free_time', 'client_free_time', 'redirect_url',
...
router.post('/routers', (req, res) => {
  ...
  `).run(name, host, parseInt(port) || 8728, username, password,
    hotspot_server || 'hotspot1', user_profile || 'inforcenter-default');
