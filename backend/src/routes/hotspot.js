...
const cpfLoginLimiter = rateLimit({
  windowMs: 10 * 60 * 1000,
  max: 8,
...
const PUBLIC_SETTINGS = [
  'hotspot_title', 'hotspot_subtitle', 'hotspot_free_time', 'client_free_time',
...
router.post('/cpf-login', cpfLoginLimiter, async (req, res) => {
  ...
  const existingLead = db.prepare('SELECT id FROM leads WHERE cpf = ?').get(digits);
  let leadId;
  if (existingLead) {
    db.prepare(`
      UPDATE leads SET name = ?, ixc_cliente_id = ?, is_client = 1,
      mac_address = ?, ip_address = ?, router_id = ?, is_validated = 1
      WHERE id = ?
    `).run(cliente.nome, cliente.id, mac || null, ip || null, mtRouter?.id || null, existingLead.id);
    leadId = existingLead.id;
  } else {
    const result = db.prepare(`
      INSERT INTO leads (name, cpf, email, ixc_cliente_id, is_client, mac_address, ip_address, router_id, is_validated)
      VALUES (?, ?, ?, ?, 1, ?, ?, ?, 1)
    `).run(cliente.nome, digits, cliente.email || null, cliente.id, mac || null, ip || null, mtRouter?.id || null);
    leadId = result.lastInsertRowid;
  }

  const freeTime = parseInt(db.prepare("SELECT value FROM settings WHERE key='client_free_time'").get()?.value) || 480;

  let mikrotikResult = null;
  try {
    mikrotikResult = await authorizeUser(mtRouter, mac, ip, digits, freeTime, 'cliente');
    db.prepare(`
  ...
  res.json({
    success: true,
    message: `Bem-vindo, ${cliente.nome.split(' ')[0]}! Acesso liberado.`,
    name: cliente.nome,
...
// POST /api/hotspot/send-otp — envia código SMS (fluxo visitante)
...
// POST /api/hotspot/connect — fluxo sem OTP (visitante ainda não-cliente): salva o lead e libera acesso direto
router.post('/connect', connectLimiter, async (req, res) => {
  ...
  let mikrotikResult = null;
  try {
    mikrotikResult = await authorizeUser(router, mac, ip, digits, freeTime, 'lead');
    db.prepare(`
      INSERT INTO sessions (lead_id, mac_address, ip_address, mikrotik_user, router_id)
      VALUES (?, ?, ?, ?, ?)
  ...
  res.json({
    success: true,
    message: 'Acesso liberado! Aproveite o Wi-Fi da Inforcenter.',
    username: mikrotikResult?.username,
    password: mikrotikResult?.password,
    freeTimeMinutes: freeTime
  });
...
// POST /api/hotspot/verify — valida OTP e libera acesso (visitante)
router.post('/verify', async (req, res) => {
  ...
  let mikrotikResult = null;
  try {
    mikrotikResult = await authorizeUser(mtRouter, mac, ip, digits, freeTime, 'lead');
    db.prepare(`
      INSERT INTO sessions (lead_id, mac_address, ip_address, mikrotik_user, router_id)
      VALUES (?, ?, ?, ?, ?)
    `).run(leadId, mac || null, ip || null, mikrotikResult?.username || null, mtRouter?.id || null);
