...
async function authorizeUser(router, macAddress, ipAddress, identifier, freeTimeMinutes, prefix = 'lead') {
  if (!router) throw new Error('Nenhum MikroTik ativo cadastrado.');
  const client = createClient(router);
  const server = router.hotspot_server || 'hotspot1';
  const username = `${prefix}_${identifier.replace(/\D/g, '')}`;
  ...
  try {
    ...
    await client.write('/ip/hotspot/user/add', [
      ...
      `=comment=${prefix === 'cliente' ? 'Cliente' : 'Lead'}-${identifier}`
    ]);
