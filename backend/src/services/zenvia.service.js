...
async function sendOTP(phone, code) {
  ...
  const from = process.env.ZENVIA_FROM || 'Inforcenter';

  // Formata o número para E.164 (Brasil)
  const formattedPhone = formatPhone(phone);
  const message = `Inforcenter: Seu codigo de acesso e ${code}. Valido por ${process.env.OTP_EXPIRY_MINUTES || 5} minutos.`;
