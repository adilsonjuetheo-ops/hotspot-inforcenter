...
let token = IS_DEMO ? 'demo-token' : localStorage.getItem('inforcenter_token');
let adminUsername = localStorage.getItem('inforcenter_admin') || 'Admin';
let currentPage = 1;
...
async function doLogin() {
  ...
  try {
    ...
    localStorage.setItem('inforcenter_token', token);
    localStorage.setItem('inforcenter_admin', adminUsername);
    showAdminApp();
  } catch (err) {
...
function logout() {
  localStorage.removeItem('inforcenter_token');
  localStorage.removeItem('inforcenter_admin');
  location.reload();
}
...
async function loadRecentLeads() {
  try {
    ...
    if (!data.leads.length) {
      tbody.innerHTML = '<tr><td colspan="4"><div class="empty-state"><div class="icon">👥</div><p>Nenhuma conexão registrada ainda.</p></div></td></tr>';
      return;
    }
    tbody.innerHTML = data.leads.map(l => `
      <tr>
        <td><strong>${l.name || '—'}</strong></td>
        <td>${tipoBadge(l.is_client)}</td>
...
function tipoBadge(isClient) {
  return isClient
    ? '<span class="badge badge-green">🪪 Cliente</span>'
    : '<span class="badge badge-gray">Visitante</span>';
}
...
async function loadLeads() {
  ...
  try {
    ...
    if (!data.leads.length) {
      tbody.innerHTML = '<tr><td colspan="7"><div class="empty-state"><div class="icon">👥</div><p>Nenhum registro encontrado.</p></div></td></tr>';
    } else {
      tbody.innerHTML = data.leads.map(l => {
        const contato = l.is_client
          ? `CPF ${maskCpf(l.cpf)}${l.email ? `<br><span class="td-muted" style="font-size:12px;">${l.email}</span>` : ''}`
          : `${formatPhone(l.phone)}${l.email ? `<br><span class="td-muted" style="font-size:12px;">${l.email}</span>` : ''}`;
        const waBtn = !l.is_client && l.phone
          ? `<a class="btn btn-sm btn-whatsapp" href="https://wa.me/55${l.phone}" target="_blank" rel="noopener" title="Conversar com ${l.name || 'o visitante'} no WhatsApp">
    ...
    document.getElementById('pagination-info').textContent = `${data.total} registros | Página ${currentPage} de ${totalPages || 1}`;
...
async function exportLeads() {
  try {
    ...
    a.href = url;
    a.download = `leads_inforcenter_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
...
async function addRouter() {
  const data = {
    ...
    user_profile: document.getElementById('router-profile').value.trim() || 'inforcenter-default'
  };
...
async function deleteRouter(id) {
  if (!confirm('Remover este MikroTik? Os registros capturados por ele são mantidos.')) return;
  try {
...
async function demoResponse(url) {
  await new Promise(r => setTimeout(r, 400));
  {
    ...
    if (url.includes('/leads')) return { total: 247, page: 1, limit: 50, leads: [
      {id:1, name:'João Silva', phone:'38988090001', email:'', cpf:'', is_client:0, mac_address:'AA:BB:CC:DD:EE:01', ip_address:'192.168.10.21', created_at:'2026-08-05T10:30:00'},
      {id:2, name:'Maria Oliveira', phone:'', email:'maria@email.com', cpf:'11240010699', is_client:1, mac_address:'AA:BB:CC:DD:EE:02', ip_address:'192.168.10.22', created_at:'2026-08-05T10:45:00'},
      {id:3, name:'Pedro Santos', phone:'38988090003', email:'', cpf:'', is_client:0, mac_address:'AA:BB:CC:DD:EE:03', ip_address:'192.168.10.23', created_at:'2026-08-05T11:00:00'},
      {id:4, name:'Ana Costa', phone:'', email:'', cpf:'22355566677', is_client:1, mac_address:'AA:BB:CC:DD:EE:04', ip_address:'192.168.10.24', created_at:'2026-08-05T11:15:00'},
      {id:5, name:'Carlos Lima', phone:'38988090005', email:'carlos@email.com', cpf:'', is_client:0, mac_address:'AA:BB:CC:DD:EE:05', ip_address:'192.168.10.25', created_at:'2026-08-05T11:30:00'},
    ]};
    if (url.includes('/sessions')) return {
      activeSessions: [
        {user:'cliente_11240010699', 'mac-address':'AA:BB:CC:DD:EE:02', address:'192.168.10.22', uptime:'00:23:14'},
        {user:'lead_38988090003', 'mac-address':'AA:BB:CC:DD:EE:03', address:'192.168.10.23', uptime:'00:05:42'},
      ],
      sessions: [
        {name:'Maria Oliveira', phone:'', mac_address:'AA:BB:CC:DD:EE:02', ip_address:'192.168.10.22', started_at:'2026-08-05T10:45:00'},
        {name:'João Silva', phone:'38988090001', mac_address:'AA:BB:CC:DD:EE:01', ip_address:'192.168.10.21', started_at:'2026-08-05T10:30:00'},
      ]
    };
    if (url.includes('/settings')) return {
      hotspot_title:'Wi-Fi Inforcenter', hotspot_subtitle:'Conecte-se e aproveite a internet mais rápida da região!',
    ...
    if (url.includes('/mikrotik/test')) return { success: true, identity: 'INFORCENTER-RB' };
